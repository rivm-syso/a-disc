logger::log_info("summarize tests: table_tests_percentage_positive_per_week")

table_tests_percentage_positive_per_week <- data_tests %>% 
  left_join(
  ...
  mutate(
    percentage_positive_tests = if_else(
      ...
      false = NA_real_)) %>% 
  select(
    week, percentage_positive_tests)
